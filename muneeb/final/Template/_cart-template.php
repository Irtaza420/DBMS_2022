...
<section id="cart" class="py-3 mb-5">
            ...
            <div class="col-sm-3">
                <div class="sub-total border text-center mt-2">
                    ...
                    <div class="border-top py-4">
                        ...
                        <form method = "post"><button type="submit" class="btn btn-warning mt-3" name = "proceed">Proceed to Buy</button> </form>
                    </div>
                </div>
            </div>
            <!-- !subtotal section-->
    ...
    foreach ($product->getData('cart') as $item1){
        print_r($product->getProduct($item1['item_id']));
    
    }
    if(isset($_POST['proceed']) && !isset($_SESSION["valid"])) {
        echo '<script>
           alert("You need to login before Proceeding!");
        </script>';
    }
    else if(isset($_POST['proceed']) && isset($_SESSION["valid"]))
    {
        $timestamp = date("Y-m-d H:i:s");
        $status = "Pending";
        $id = $_SESSION["id"] ;
        $tot = $Cart->getSum($subTotal);
        $connect = mysqli_connect("localhost", "root", "") or die(mysqli_error($connect));
        mysqli_select_db($connect,"shopee") or die(mysqli_error($connect));
        $timestamp = date("Y-m-d H:i:s");
        $query = "Insert into orders (order_date, total_price, user_id, status)  Values ('$timestamp','$tot','$id','$status');";
        $result = mysqli_query($connect,$query) or die(mysqli_error($connect));
        $cart->deleteCart();
        header ("Location:/final/thankyoupage.php" );
